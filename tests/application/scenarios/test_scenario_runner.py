...
from zalp.application.scenarios import Step, Scenario, ScenarioRunner
...
def test_scenario_runner():
    app = DummyApp()
    scenario_runner = ScenarioRunner(app)
    ...
    scenario_instance = scenario_runner.load_scenario(scenario)
    assert isinstance(scenario_instance, scenario)

    thread = scenario_runner.run_scenario()
    thread.join()
